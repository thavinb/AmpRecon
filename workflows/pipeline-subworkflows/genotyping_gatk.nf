...
workflow GENOTYPING_GATK {
  ...
  main:
    ...
    input_sample_tags_bams_indexes // tuple (sample_tag, bam_file, bam_index)
        ...
        | set{bqsr_input} // tuple(sample_tag, bam_file, bam_index, reference_fasta)

    bqsr(bqsr_input)

    // haplotype caller
    bqsr.out // tuple (sample_tag, recalibrated_bam)
        | join(sample_tag_reference_files_ch) // tuple(sample_tag, recalibrated_bam, reference_fasta, snp_list)
        | map{ it -> tuple(it[0], it[1], it[2])}
        | set{haplotype_caller_input} // tuple(sample_tag, recalibrated_bam, reference_fasta)
    ...
    gatk_haplotype_caller_gatk4.out.vcf_file_and_index
        | join(sample_tag_reference_files_ch) // tuple(sample_tag, vcf_file, vcf_index, reference_fasta, snp_list)
