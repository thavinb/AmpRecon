...
workflow IRODS {
    take:
        ...
        reference_ch // tuple ([fasta], panel_name, [fasta_idx_files])
    main:
        ...
        irods_ch
             | combine(reference_ch,  by: 1) // tuple (primer_panel, sample_id, WG_lane, irods_path, [fasta], [fasta_idx_files])
             | map { it -> tuple(it[2], it[1], it[4][0], it[5], it[0]) }
             | set{ sample_id_ref_ch } // tuple (WG_lane, sample_id, fasta_file, fasta_idx, primer_panel)
