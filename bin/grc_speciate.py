...
class Speciate:
    def __init__(
        ...
        self.write_out = {}
        self.species_order = species_order # sets the order in which species should be written
        #run main logic flow
    ...
    def _get_species_tag(self, barcode:str, default_species:str, min_coverage: int) -> str:
        ...
        """
        def __reorder_list(values:list, ordering:list) -> list:
            '''
            reorder a given list of values according to a prespecified ordering list.
            It will raise an error, if a value is not present in the ordering list provided
            '''
            # TODO: write unit tests for this function
            # if just a single value or an empty list, just return it
            if len(values) <= 1:
                return values
            # if more than one item, reorder it.
            if len(values) > 1:
                # Create a dictionary to map each value to its index in the ordering
                index_map = {value: index for index, value in enumerate(ordering)}

                # Check if all values in the input list are valid
                for value in values:
                    if value not in index_map:
                        raise ValueError(f"{value} is not a valid value in the ordering list provided {ordering}.")

                # Sort the values using the index map as the key
                sorted_values = sorted(values, key=lambda value: index_map.get(value, len(ordering)))
