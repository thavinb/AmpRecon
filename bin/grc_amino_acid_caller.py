...
class AminoAcidCaller:  # better name
    ...
    def _read_drl_info(self, filename):
        ...
        for row in self._read_tsv_row_generator(filename):
            ...
            if row["AA_Pos"] not in ret["pos"][row["GeneName"]]:
                ret["pos"][row["GeneName"]][row["AA_Pos"]] = {
                    "1": row["pos1"],
                    "2": row["pos2"],
                    "3": row["pos3"],
                }
    ...
    def call_haplotypes(self):
        ...
        for id in sample_id:
            ...
            for gene_amino in drl["exp_order"]:
                ...
                if (
                    gene
                    in self.config["grc_amino_acid_caller"][
                        "DR_HAPLOTYPE_SPECIAL_CASES"
                    ]
                ):
                    if (
                        amino
                        ...
                        aa = self.config["grc_amino_acid_caller"][
                            "DR_HAPLOTYPE_SPECIAL_CASES"
                        ][gene][amino]
                ...
                if (
                    self.is_missing(base_1)
                    or self.is_missing(base_2)
                    or self.is_missing(base_3)
                ):
                    if gene_amino in core_genes:
                        self.haplotypes[id][gene].append("-")
                    self.haplotypes[id][gene_amino].append("-")
                ...
                if (
                    ...
                    try:
                        het_call = self._get_het_match_from_config(
                            gene, amino, base_1, base_2, base_3
                        )
                        ...
                        self.haplotypes[id][gene_amino].append("-")
                        print(
                            f"Double heterozygous case present in data not in list of special cases accounted for by the pipeline, please check input data at the locus: {gene_amino} for sample: {id}"
                        )
                        continue
        ...
        for sample in self.haplotypes:
            for gene_amino in drl["exp_order"]:
                ...
                if (
                    ...
                    num_hc_for_gene_in_conf = len(
                        self.config["grc_amino_acid_caller"][
                            "DR_HAPLOTYPE_SPECIAL_CASES"
                        ][gene]
                    )
    ...
    def _get_het_match_from_config(self, gene, amino, base_1, base_2, base_3):
        ...
        if gene in self.config["grc_amino_acid_caller"]["DR_DOUBLE_HETEROZYGOUS_CASES"]:
            for case in self.config["grc_amino_acid_caller"][
                "DR_DOUBLE_HETEROZYGOUS_CASES"
            ][gene]:
...
class HaplotypeProcessingException(Exception):
    def __init__(
        self,
        message="Error in processing genotype information, please check the input data",
    ):
...
class InputAmpliconFormattingException(Exception):
    def _init__(
        self,
        message="Data contains nucleotide information that was not expected, expected base data is: A,T,G,C or '-'",
    ):
        self.message = message
...
if __name__ == "__main__":
    ...
    caller = AminoAcidCaller(
        args.genotype_files, args.config, args.drl_information_file, args.codon_key_file
    )
    output = caller.call_haplotypes()
    write_out_grcs(
        output, caller.drl_info, args.output_grc1_file, args.output_grc2_file
    )
