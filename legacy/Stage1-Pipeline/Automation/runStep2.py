...
def runStep2ForASample(_executionPath,_refPath, _savePath, _index,_name,_batch):
 ...
 for c in calls:
  ...
  if c == "5-bwa-align" or c == "12-scramble2" or c == "7-reheader":
   args.append("\"{}\"".format(_refPath))

  if not executeStep(c,args,_executionPath):
   print("Step 2-{} Failed".format(count))
   return False
...
def runStep2(_manifest,_executionPath,_refPath, _savePath):
 ...
 stepPaths = ["step2-{}".format(l) for l in range(1,13)]
 ...
 with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
  for sample in _manifest.data:
   ...
   sampleTasks.append(executor.submit(runStep2ForASample, _executionPath,refPath, _savePath, sample["index"], sample["sims"],_manifest.batch))
...
def main():
 ...
 if len(sys.argv) < 3:
  exitWithError("Missing Step 2 scripts folder")
 ...
 result = runStep2(man,sys.argv[2],sys.argv[3],sys.argv[4])
 if not result[0]:
...
if __name__ == "__main__":
 ...
 print("Runs step 2 of the Stage 1 Pipeline (Perform science!)")
